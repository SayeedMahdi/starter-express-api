...
const mapToTree = (list, parentId = null) => {
  ...
  list.forEach(item => {
    const node = { ...item }
    node["replyTo"] = Object.prototype.hasOwnProperty.call(node, "replyTo") ? node["replyTo"] : null
...
BlogSchema.methods.findRemoveOrphanComments = function () {
  ...
  this.comments.forEach(comment => {
    if (comment.replyTo) {
      const parentDocExists = this.comments.find(_comment => _comment["_id"].toString() === comment["replyTo"].toString()) && true;
      if (!parentDocExists) {
