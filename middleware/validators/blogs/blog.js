...
const createBlogSchema = checkSchema({
  ...
  slug: {
    ...
    custom: {
      errorMessage: (_, { req }) => req.t("already-exists", { ns: 'validations', key: req.t("title") }),
      options: (slug, { req }) => {
  ...
  excerpt: {
    ...
    isEmpty: {
      negated: true,
      errorMessage: (_, { req }) => req.t("required", { ns: 'validations', key: req.t("excerpt") }),
    }
  },
  content: {
    isEmpty: {
...
const restoreBlogSchema = checkSchema({
  id: {
    ...
    isEmpty: {
      negated: true,
      errorMessage: (_, { req }) => req.t("required", { ns: 'validations', key: req.t('item-id', { key: "Blog" }) })
    },
  },
});
